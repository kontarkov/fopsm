...
1. Процесс cp orel-table.iso (PID 8118) активно выполняет операцию копирования, записывая 5.24 ГБ данных. Это основная операция, которая использует диск, как видно из высокой активности записи.  
2. Процесс [jbd2/sda2-8] (PID 556) связан с журналированием в файловой системе ext4 (параметры устройства sda2), где происходит запись данных в журнал файловой системы для обеспечения целостности данных. Он записывает 328.00 КБ данных. Это фоновая операция, связанная с обслуживанием файловой системы.  
3. Процессы [kworker/u128:1+flush-8:0], [kworker/u128:4-events_unbound] и другие kworker процессы:  
-- Это фоновые задачи ядра, которые занимаются управлением устройства ввода/вывода, синхронизацией данных и другими операциями, связанными с обслуживанием дисков и файловых систем.  
-- Эти процессы также записывают данные на диск, но с меньшей интенсивностью, чем основной процесс копирования.  
