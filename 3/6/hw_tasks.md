...
__`iostat`__  
  ...
  Процессор использует около 7-8% времени на пользовательские процессы (столбцы `%user`), и 63-66% времени процессор находится в ожидании ввода/вывода (`%iowait`), что может указывать на то, что диск не успевает справляться с нагрузкой.
...
_Открываем всё больше и больше ресурсоёмких страниц в веб-браузере._  
_Мониторинг памяти по `free -h`._  
![swappiness70_1](./images/6_8_1.png)  

_АНАЛИЗ_  
В сравнении с предыдущими значениями `vm.swappiness`, на скриншоте видно, что swap используется раньше. Доступная память существенно ниже.  
  
`vm.swappiness=70` приводит к более активному использованию swap.  
В случае `vm.swappiness=30` и `vm.swappiness=50`, система использует swap экономнее.
При `vm.swappiness=30` минимальное использование swap и система производительнее.
