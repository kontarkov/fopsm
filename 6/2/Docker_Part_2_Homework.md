...
---
# Домашнее задание «Docker. Часть 2»
__ШТЕНГЕЛОВ ИГОРЬ__
...
Для контейнера Prometheus был подключён каталог с конфигурацией ./prometheus/. Благодаря этому при запуске использовался подготовленный файл prometheus.yml, содержащий необходимые настройки сбора метрик, включая подключение Pushgateway. Именованный том `prometheus_data` предназначен для хранения данных Prometheus независимо от жизненного цикла контейнера. Публикация `9090:9090` делает веб-интерфейс Prometheus доступным с Docker-сервера.
