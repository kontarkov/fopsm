...
После корректировки CPU-модели Alpine успешно загрузилась и дошла до приглашения входа в систему:
...
Для сравнения времени запуска обе уже установленные VM были приведены к одинаковому исходному состоянию `shut off`. Затем каждая VM запускалась командой `virsh start ... --console`. Время фиксировалось на Ubuntu-хосте непосредственно перед стартом VM и сразу после появления приглашения входа в систему.
...
Наблюдаемое время до приглашения входа в систему: **43 секунды**.
...
Наблюдаемое время до приглашения входа в систему: **11 секунд**.

![Замер времени запуска KVM](images3/02_kvm_boot_timing.png)

## 3. Сравнение результатов

| Режим | Время до приглашения входа в систему |
|---|---:|
| QEMU без KVM-ускорения | 43 с |
| KVM | 11 с |

Разница составила 32 секунды. В данном конкретном nested-стенде KVM дошёл до приглашения входа в систему примерно в **3,9 раза быстрее**, а наблюдаемое время загрузки уменьшилось примерно на **74%**.
