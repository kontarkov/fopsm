...
В Ubuntu проверены флаг аппаратной виртуализации Intel, устройство KVM и пакеты libvirt:
...
Подтверждены `vmx` и наличие `/dev/kvm`.
...
Для сравнения времени запуска обе уже установленные VM были приведены к одинаковому исходному состоянию `shut off`. Затем каждая VM запускалась командой `virsh start ... --console`. Время фиксировалось на Ubuntu-хосте непосредственно перед стартом VM и сразу после появления serial login prompt.
...
Это не универсальный коэффициент производительности: результат относится к данному стенду, версиям ПО и конкретной конфигурации VM.

При установке Alpine также субъективно наблюдалась более высокая скорость работы KVM, но интерактивная установка не измерялась как строгий benchmark, поскольку в её общее время входит ввод ответов пользователем. Поэтому количественное сравнение выполнено по одинаково организованному запуску уже установленных VM.
...
В выполненном сравнении KVM показал существенно меньшее время запуска гостевой Alpine Linux, чем QEMU без аппаратного ускорения: 11 с против 43 с до появления login prompt. Для данного лабораторного стенда результат соответствует ожидаемому преимуществу аппаратной виртуализации KVM перед программным выполнением QEMU.
